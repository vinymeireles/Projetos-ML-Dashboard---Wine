...
wine_df["WineType"] = [wine.target_names[tpy] for tpy in wine.target]

## Treinar e dividir o modelo (treinamento e teste)
...
#Titulos
st.title("🍷Tipos de Vinhos :red[Prediction] :chart_with_upwards_trend:")
st.markdown("Predict Wine Type using Ingredients Values")

# 3 Guias (abas) layout - tab1, tab2, tab3
...
with tab2:
    st.header("Matriz de Confusão |Feature Importances")
    col1, col2 = st.columns(2)      #criando 2 colunas para os gráficos (ficarem lado a lado)
    ...
    st.code(classification_report(Y_test, Y_test_preds))   #st.code função para escrever qualquer código em Python, Java e exibir na pagina web...

with tab3:
    #adicionar controle deslizantes para os ingredientes do vinho para análise de performance.
    sliders = []  
    ...
    with col2:
        ...
        with col2:
            st.metric(label = "Model Confidence", value= "{:.2f} %".format(probability*100), delta="{:.2f} %".format((probability-0.5)*100)) #calcular a porcentagem e o delta = intervalo de confiança(quanto acima ou abaixo) a métrica do modelo é confiável.
        ...
        intepretation_fig = explanation.as_pyplot_figure(label=prediction[0])
        st.pyplot(intepretation_fig, use_container_width=True)
